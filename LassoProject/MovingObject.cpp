...
void MovingObject::nextStep(double t) {
  ...
  for(size_t i=0; i<parts.size(); i++){
    ...
    parts[i]->move(xCurrent, yCurrent);
  }
  vx += ax*t;
  vy += ay*t;
...
void MovingObject::getAttachedTo(MovingObject *m) {
  ...
  initMO(m->vx, m->vy, m->ax, m->ay, m->paused); /// Arpon Note: This is a method of Sprite. It initializes moving object. Here moving object is coin and coin is getting paused.
}
